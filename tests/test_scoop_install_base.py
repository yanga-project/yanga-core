...
from yanga_core.steps.scoop_install_base import ScoopInstall, ScoopInstallExecutionInfo, ScoopManifest
...
def test_scoop_install_run_skips_on_non_windows(tmp_path: Path) -> None:
    ...
    with patch("yanga_core.steps.scoop_install_base.platform.system", return_value="Linux"):
        assert scoop_install.run() == 0

    with patch("yanga_core.steps.scoop_install_base.platform.system", return_value="Darwin"):
        assert scoop_install.run() == 0
...
def test_scoop_install_run_executes_on_windows(tmp_path: Path) -> None:
    ...
    with patch("yanga_core.steps.scoop_install_base.platform.system", return_value="Windows"):
        assert scoop_install.run() == 0
