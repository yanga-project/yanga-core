...
def populated_project(tmp_path: Path) -> Path:
    build_dir = tmp_path / ".yanga" / "build" / "MyVariant" / "host" / "Debug"
...
def test_pristine_wipes_targeted_variant_build_dir(populated_project: Path) -> None:
    target = populated_project / ".yanga" / "build" / "MyVariant" / "host" / "Debug"

    RunCommand._run_pristine(RunCommandConfig(project_dir=populated_project, pristine=True, variant_name="MyVariant", platform="host", build_type="Debug"))

    assert not target.exists()
    assert (populated_project / ".yanga" / "build" / "MyVariant" / "host").exists()


def test_pristine_with_no_scope_wipes_entire_yanga_build(populated_project: Path) -> None:
    RunCommand._run_pristine(RunCommandConfig(project_dir=populated_project, pristine=True))

    assert not (populated_project / ".yanga" / "build").exists()


def test_pristine_is_a_no_op_if_target_does_not_exist(tmp_path: Path) -> None:
    RunCommand._run_pristine(RunCommandConfig(project_dir=tmp_path, pristine=True, variant_name="Ghost"))
...
def test_pristine_refuses_to_escape_build_root(bad_variant: str, tmp_path: Path) -> None:
    with pytest.raises(UserNotificationException, match="escapes yanga build root"):
        RunCommand._run_pristine(RunCommandConfig(project_dir=tmp_path, pristine=True, variant_name=bad_variant))
