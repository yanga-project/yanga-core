...
@dataclass
class ReportRelevantHtmlContent(BaseConfigJSONMixin):
    """
    Used to register generated html content relevant for the report.

    This content will be referenced in the `toctree` like `My Report <./path/index.html#http://>`.
    This is useful for documentation tools that generate html output directly.
    ...
    #: Name to be displayed in the toctree
    name: str
    #: Relative path to the generated html index file (may have any name, not necessarily index.html)
    index_html: Path
...
class ReportRelevantFiles(BaseConfigJSONMixin):
    ...
    #: Generated html content that is relevant for the report and shall be referenced as-is
    html_content: ReportRelevantHtmlContent | None = None
...
class VariantReportData(BaseConfigJSONMixin):
    ...
    @property
    def html_content(self) -> list[ReportRelevantHtmlContent]:
        return [file.html_content for file in self.files if file.html_content]
