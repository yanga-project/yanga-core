...
from yanga_core.domain.reports import FeaturesReportRelevantFile, ReportRelevantFiles, ReportRelevantFileType
...
@dataclass
class KConfigGenConfig(DataClassDictMixin):
    kconfig_model_file: str = "KConfig"
...
class KConfigGen(PipelineStep[ExecutionContext]):
    def __init__(self, execution_context: ExecutionContext, group_name: str | None = None, config: dict[str, Any] | None = None) -> None:
        ...
        self.user_config = KConfigGenConfig.from_dict(config) if config else KConfigGenConfig()
    ...
    def kconfig_model_file(self) -> Path:
        """Path to the KConfig model file, configurable via 'kconfig_model_file' in the step config."""
        return self.project_root_dir.joinpath(self.user_config.kconfig_model_file)

    def run(self) -> int:
        self.logger.debug(f"Run {self.get_name()} stage. Output dir: {self.output_dir}")
        kconfig_model_file = self.kconfig_model_file
